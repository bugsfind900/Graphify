...
BACKENDS: dict[str, dict] = {
    ...
    "litellm": {
        "base_url": os.environ.get("LITELLM_BASE_URL", "http://0.0.0.0:8000"),
        "default_model": os.environ.get("LITELLM_MODEL", "gpt-4o"),
...
def _call_litellm(
    ...
    try:
        ...
        print(f"[graphify] litellm error: {exc}", file=sys.stderr)
...
def extract_files_direct(
    ...
    if backend == "litellm":
        return _call_litellm(cfg["base_url"], key, mdl, user_msg, temperature=cfg.get("temperature", 0))
...
def detect_backend() -> str | None:
    ...
    """
    for backend in ("gemini", "kimi", "claude", "openai", "deepseek", "litellm"):
